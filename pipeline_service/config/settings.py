...
class SpeculativeConfig(BaseModel):
    """Speculative-decoding block for an SGLang endpoint.

    Mirrors `--speculative-*` on `sglang.launch_server`. The vLLM engine keeps
    expressing speculation through `extra_args` (`--speculative-config <json>`),
    because that is one opaque JSON blob rather than a set of flags.
    """

    algorithm: str = "DFLASH"                 # DFLASH | EAGLE | NGRAM | ...
    draft_model: str = ""                     # HF repo id of the drafter
    draft_revision: str | None = None
    num_draft_tokens: int | None = None       # DFLASH: whole block (8 == vLLM's 7)
    num_steps: int | None = None              # EAGLE/MTP chain length
    eagle_topk: int | None = None


class VllmServeConfig(BaseModel):
    """
    Options for launching a local inference server via run.sh / llm.spawn.

    Historically vLLM-only, hence the name and the `vllm:` config key. `engine`
    now selects the backend; every field below that has an SGLang equivalent is
    translated by `llm/spawn.py::_build_cmd` (see the mapping table there).
    """
    ...
    # engine="sglang": the venv python that runs `-m sglang.launch_server`.
    engine_python: str | None = None         # default /opt/sglang-env/bin/python
    trust_remote_code: bool = False
    reasoning_parser: str | None = "qwen3"   # null/"" -> omit --reasoning-parser
    speculative: SpeculativeConfig | None = None
    # Extra process env for this endpoint only (e.g. SGLANG_OPT_* tuning
    # switches, HF_TOKEN overrides). Merged over the inherited environment.
...
class ActorConfig(BaseModel):
    """Per-actor config."""
    # judge only: false skips the bracket entirely. With ensemble_size > 1 the
    # candidates are still all generated and validated; the first one that passes
    # is promoted. Used for dataset generation, where there is nothing to rank.
    enabled: bool = True
