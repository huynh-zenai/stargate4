...
class Candidate:
    ...
    # Telemetry (run analysis only — never read by the pipeline itself).
    coder_s: float = 0.0            # wall time inside the coder call
    validate_s: float = 0.0         # js-check + render + judge-view/embed time
    code_chars: int = 0
    code_sha: str | None = None     # first 12 hex of the sha256 used for dedup
    render_ms: float | None = None
    usage: dict[str, Any] = field(default_factory=dict)   # tokens for this candidate's coder session
