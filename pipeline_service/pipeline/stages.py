...
async def _resolve_bracket(
    *,
    ...
    async def _judge_duel(left: Candidate, right: Candidate, round_no: int) -> Candidate:
        ...
        _t_e = time.monotonic()
        # Telemetry: one record per duel so a run can be replayed offline —
        # who met whom, which stage decided it, and what each stage saw.
        task.meta.setdefault("duels", []).append({
            "round": round_no,
            "label": label,
            "k_a": left.k, "k_b": right.k,
            "winner_k": winner.k,
            "winner_side": verdict.winner,
            "decided_by": verdict.decided_by,
            "confidence": verdict.confidence,
            "reason": verdict.reason,
            "queue_wait_s": round(_t_s - _t_q, 2),
            "compare_s": round(_t_e - _t_s, 2),
            "stages": {
                k: verdict.detail.get(k)
                for k in ("s1_slim", "s2_slim", "s3_slim", "s4_slim")
                if verdict.detail.get(k) is not None
            },
        })
        logger.info(
...
class _CandidateFactory:
    ...
    async def _generate(self, k: int) -> Candidate:
        ...
        cand.coder_s = time.time() - t0
        cand.elapsed_s = cand.coder_s
        if cand.js_code:
            cand.code_chars = len(cand.js_code)
            cand.code_sha = _code_digest(cand.js_code)[:12]
        session = self.coder.session_store.get(task.stem, f"coder#k{k}")
        if session is not None:
            cand.usage = dict(getattr(session, "usage", {}) or {})
        return cand
    ...
    async def _validate(self, cand: Candidate) -> None:
        ...
        async with self.sem_checker:
            try:
                ...
                cand.validate_s = time.time() - t0
                cand.elapsed_s += cand.validate_s
                return
        ...
        cand.render_ms = shadow.render_ms
...
async def multigen_first_iter(
    ...
    *,
    ...
    # Telemetry: full per-candidate record. `duels` was appended by _judge_duel.
    task.meta["multigen"] = {
        "K": K,
        "temperature": ensemble_temperature,
        "seed_base": task.seed,
        "multimodal": send_image,
        "unique_programs": unique,
        "survivors": [c.k for c in survivors],
        "winner_k": winner.k,
        "n_duels": len(task.meta.get("duels", [])),
        "candidates": [
            {
                "k": c.k,
                "seed": c.seed,
                ...
                "js_valid": c.js_valid,
                "js_errors": c.js_errors,
                "render_errors": c.render_errors,
                "code_chars": c.code_chars,
                "code_sha": c.code_sha,
                "coder_s": round(c.coder_s, 2),
                "validate_s": round(c.validate_s, 2),
                "elapsed_s": round(c.elapsed_s, 2),
                "render_ms": c.render_ms,
                "usage": c.usage,
                "rendered": c.rendered_png is not None,
                "has_judge_views": bool(c.judge_white_views),
            }
            for c in candidates
        ],
    }
