...
async def debug_task(
    stem: str,
    include_candidate_pngs: bool = True,
    include_candidate_js: bool = False,
):
    """Full task payload.

    `include_candidate_pngs=false` drops the per-candidate and refinement grids —
    at ensemble 32 those are ~32 base64 PNGs per stem, so a batch-wide harness
    pull is an order of magnitude smaller without them.

    `include_candidate_js=true` adds every candidate's source keyed by k, losers
    included. Only the winner's code survives into /results, so this is the only
    way to see what the bracket rejected. Text, so it costs a few hundred KB per
    stem rather than the hundreds of MB the PNGs cost."""
    ...
    return {
        ...
        ] if (task.multigen_pngs and include_candidate_pngs) else None),
        "refinement_rendered_pngs_b64": ([
            base64.b64encode(png).decode() if png else None
            for png in (task.refinement_rendered_pngs or [])
        ] if (task.refinement_rendered_pngs and include_candidate_pngs) else None),
        "candidate_js": ({
            str(c.k): c.js_code for c in (task.candidates or []) if c.js_code
        } if include_candidate_js else None),
        "refinement": {
...
@app.get("/debug/run")
async def debug_run():
    """Batch- and host-level telemetry for offline run analysis.

    Everything here is already used elsewhere (miner-diag header, preflight
    metrics, coder probe); this endpoint just exposes it in one JSON so a run
    harness can snapshot it per batch without scraping the log.
    """
    now = time.time()
    return {
        "status": state.status.value,
        ...
        "batch_wall_s": (round(now - state.batch_started_at, 2)
                         if state.batch_started_at else None),
        "diag": state.diag,
        "llm_status": {k: v.value for k, v in state.llm_status.items()},
        "failed": state.failed,
        "diag_header": _diag_header().strip(),
        "config": {
            "ensemble_size": settings.actors.coder.ensemble_size,
            "coder_model": settings.actors.coder.model,
            "coder_workers": settings.actors.coder.workers,
            "coder_temperature": settings.actors.coder.ensemble_temperature,
            "judge_model": settings.actors.judge.model,
            "judge_workers": settings.actors.judge.workers,
            "seed_offset": settings.pipeline.seed_offset,
            "img_size": settings.renderer.img_size,
            "refinement_enabled": settings.pipeline.refinement_enabled,
            "max_iter": settings.event_bus.max_iter,
        },
    }
