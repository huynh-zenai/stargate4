...
class RendererModule(BaseModule):
    ...
    @property
    def _ping_grace_s(self) -> float:
        """How long an unanswered /ping is tolerated before declaring death.

        Derived from the render budget so it scales with the configured
        workload instead of the host: a sidecar may block a full render
        timeout on one request and still be perfectly healthy.
        """
        return self.config.render_timeout_ms / 1000.0 + self._PING_GRACE_MARGIN_S
    ...
    async def _monitor(self, sc: _Sidecar) -> None:
        ...
        while True:
            ...
            try:
                ...
                last_reason = "" if ok else f"status={r.status_code}"
            except Exception as exc:
                ok = False
            ...
            if ok:
                if unhealthy_since is not None:
                    logger.info(
                        f"[RENDERER] sidecar #{sc.idx} answering again after "
                        f"{time.monotonic() - unhealthy_since:.0f}s busy/unreachable"
                    )
            ...
            if unhealthy_since is None:
                unhealthy_since = now
            grace = self._ping_grace_s
            down_for = now - unhealthy_since
            if down_for >= grace:
                logger.warning(
                    f"[RENDERER] sidecar #{sc.idx} unreachable for {down_for:.0f}s "
                    f"(> {grace:.0f}s grace, last: {last_reason}) — restarting"
                )
                return
    ...
    async def _await_sidecar(self) -> _Sidecar | None:
        """A live sidecar, waiting out a momentarily empty pool.

        Every slot is supervised and respawns on its own, so an empty pool is
        normally transient — a restart in flight, not a dead service. Failing
        instantly would discard a candidate whose code already cost a full
        coder generation, so it is always cheaper to wait. Bounded by the
        request timeout the caller was already prepared to spend.
        """
        ...
        while time.monotonic() < deadline:
            ...
            if not warned:
                logger.warning(
                    "[RENDERER] no live sidecar — waiting up to "
                    f"{self.config.request_timeout_s:.0f}s for one to come back"
                )
                warned = True
            await asyncio.sleep(1.0)
            sc = self._next_sidecar()
            if sc is not None:
                logger.info(f"[RENDERER] sidecar #{sc.idx} available again — resuming")
                return sc
        return None
    ...
    async def _post_with_retry(self, path: str, payload: dict) -> tuple[httpx.Response, int]:
        ...
        try:
            ...
            first_idx = sc.idx
            retry_sc = await self._await_sidecar()
