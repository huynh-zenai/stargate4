...
HF_TOKEN="${HF_TOKEN:-}" MODEL="$MODEL" MODEL_REVISION="$MODEL_REVISION" \
SKIP_CUDA_CHECK="${SKIP_CUDA_CHECK:-0}" "$VENV/bin/python" - <<'PY'
import os, torch, vllm, fastapi
from transformers import AutoProcessor, __version__ as tv
from transformers.processing_utils import ProcessorMixin
if os.environ.get("SKIP_CUDA_CHECK") not in ("1", "true", "yes"):
    torch.zeros(1).cuda()  # raises if the torch/vllm CUDA build mismatches the driver
else:
    print("[glm-env] SKIP_CUDA_CHECK set — no GPU here, deferring the CUDA check to runtime")
